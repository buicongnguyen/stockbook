...
function BookBlock({ block, index }: { block: Block; index: number }) {
  ...
  if (block.type === "h3") return <h2 id={anchor} className="section-heading scroll-anchor">{block.text}</h2>;
  if (block.type === "h4") return <h3 id={anchor} className="subsection-heading scroll-anchor">{block.text}</h3>;
  ...
  if (isSection) return <h2 id={anchor} className="section-heading inferred scroll-anchor">{block.text}</h2>;
  if (isGroup) return <h3 id={anchor} className="group-heading scroll-anchor">{block.text.replace(/^[-]\s*/, "")}</h3>;
  if (isNumberedItem) return <h3 id={anchor} className="numbered-heading scroll-anchor">{block.text}</h3>;
